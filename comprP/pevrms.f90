...
subroutine pevrms(vrms,isol)
!!use sepmodulemain
use sepran_arrays
use coeff
use geometry
implicit none
real(kind=8),intent(inout) :: vrms
integer,intent(in) :: isol
real(kind=8) :: volint,outval(3),vrms2
integer :: ihelp,iinvol(5)

call sepactsolbf1(isol)
call pevrms00(isol,iuser_here,user_here)
!vrms2 = volint(0,1,1,kmesh1,kprob1,isol,iuser_here,user_here,ihelp)/volume
!vrms=sqrt(vrms/volume)
iinvol=0
call integr(iinvol,outval,kmesh1,kprob1,isol,iuser_here,user_here)
